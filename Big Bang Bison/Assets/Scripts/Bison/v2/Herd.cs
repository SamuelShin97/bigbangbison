...
public class Herd : MonoBehaviour
{
    ...
    [Range(1f, 100f)]
    public float maxSpeed = 10f; // the max acceleration
    [Range(1f, 20f)]
    ...
    [Range(0, 10)]
    public int idleTimer = 2;  // the amount of time in seconds bison have to be away from the player for to go idle
    [Range(0, 10)]
    ...
    void Update()
    {
        ...
        foreach (HerdAgent agent in agents)
        {
            ...
            if (!Physics.Raycast(agent.transform.position, -Vector3.up, out ground, bisonHeight + 5f) && false)
            {
            ...
            } else
            {
                ...
                adjustment.y = (bisonHeight-0.45f) - ground.distance;
    ...
    List<Transform> GetNearbyObjects(HerdAgent agent)
    {
        ...
        foreach (Collider c in contextColliders)
        {
            ...
            if (c.gameObject.layer == 11) // check if the player is near
            {
                ...
                if (agent.state == 0) agent.Drove(); // start the droving coroutine on agent
            }
        }
        return context;
    }
