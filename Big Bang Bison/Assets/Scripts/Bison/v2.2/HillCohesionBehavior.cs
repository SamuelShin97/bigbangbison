...
[CreateAssetMenu(menuName = "Herd/Behavior/HillCohesion")]
public class HillCohesionBehavior : FilteredHerdBehavior
{
    public override Vector3 CalculateMove(HerdAgent agent, List<Transform> context, Herd herd)
    {
        // if no neighbors return no adjustment
        if (context.Count == 0 || agent.growth >= agent.fullMaturity) return Vector3.zero;
